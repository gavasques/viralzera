...
export default function Canvas() {
  ...
  const { data: canvasItems = [], isLoading } = useQuery({
    ...
    queryFn: () => neon.entities.Canvas.list('title', 200),
  });

  const { data: folders = [] } = useQuery({
    queryKey: ['canvasFolders'],
    queryFn: () => neon.entities.CanvasFolder.list('name', 100),
  });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => neon.entities.Canvas.update(id, data),
    onSuccess: () => {
  ...
  const togglePinMutation = useMutation({
    mutationFn: ({ id, isPinned }) => neon.entities.Canvas.update(id, { is_pinned: !isPinned }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['canvas-items'] });
    }
  });
  ...
  const toggleArchiveMutation = useMutation({
    mutationFn: ({ id, isArchived }) => neon.entities.Canvas.update(id, { is_archived: !isArchived }),
