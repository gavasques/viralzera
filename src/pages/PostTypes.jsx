...
export default function PostTypes() {
  ...
  const saveTypeMutation = useMutation({
    mutationFn: (data) => {
      ...
      if (editingType?.id) return base44.entities.PostType.update(editingType.id, payload);
      return base44.entities.PostType.create(payload);
    },
    onSuccess: () => {
  ...
  const deleteTypeMutation = useMutation({
    mutationFn: (id) => base44.entities.PostType.delete(id),
    onSuccess: () => {
  ...
  const addExampleMutation = useMutation({
    mutationFn: (newExample) => {
      ...
      return base44.entities.PostType.update(editingType.id, { examples: updatedExamples });
    },
    onSuccess: () => {
  ...
  const toggleActiveStatus = useCallback(async (type) => {
    ...
    try {
      await base44.entities.PostType.update(type.id, { is_active: newStatus });
      queryClient.invalidateQueries({ queryKey: ['postTypes', selectedFocusId] });
