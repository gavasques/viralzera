...
export default function Trends() {
  ...
  const { data: trendConfig } = useQuery({
    ...
    queryFn: async () => {
      const configs = await neon.entities.TrendConfig.list('-created_date', 1);
  ...
  const saveTrendMutation = useMutation({
    mutationFn: (data) => neon.entities.Trend.create(data),
    onSuccess: () => {
  ...
  const deleteTrendMutation = useMutation({
    mutationFn: (id) => neon.entities.Trend.delete(id),
    onSuccess: () => {
  ...
  const handleSearch = async () => {
    ...
    if (!trendConfig?.config?.search_model) return toast.error("Configure o modelo de busca em Admin Zone > Configurações de Agentes");

    setIsSearching(true);
    setSearchResults(null);

    try {
      const tc = trendConfig?.config || {};
      const prompt = (tc.default_prompt || getDefaultPrompt())
        .replace("{ASSUNTO}", subject.trim())
        .replace("{PALAVRA_CHAVE}", keyword.trim() || subject.trim());

      const result = await searchTrends({
        model: tc.search_model,
        prompt,
        maxTokens: tc.max_tokens || 32000,
      });
