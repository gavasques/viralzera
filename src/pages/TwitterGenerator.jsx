...
export default function TwitterGenerator() {
  ...
  const { data: savedConfig } = useQuery({
    ...
    queryFn: async () => {
      ...
      const configs = await neon.entities.TwitterConfig.filter({ created_by: user.email });
  ...
  const { data: project, isLoading: isLoadingProject } = useQuery({
    ...
    queryFn: () => projectId ? neon.entities.TwitterProject.get(projectId) : null,
    enabled: !!projectId
  });
  ...
  const saveConfigMutation = useMutation({
    mutationFn: async (data) => {
      if (savedConfig?.id) {
        return neon.entities.TwitterConfig.update(savedConfig.id, data);
      }
      return neon.entities.TwitterConfig.create(data);
    },
    onSuccess: () => {
  ...
  const saveProjectMutation = useMutation({
    mutationFn: async (data) => {
      ...
      if (idToUpdate) {
        return neon.entities.TwitterProject.update(idToUpdate, payload);
      }
      return neon.entities.TwitterProject.create(payload);
    },
  ...
  const handleImagePaste = useCallback(async (e) => {
    ...
    for (const item of items) {
      if (item.type.startsWith('image/')) {
        ...
        toast.promise(
          neon.integrations.Core.UploadFile({ file }).then(({ file_url }) => {
  ...
  const handleAddImage = async (e) => {
      ...
      const promise = neon.integrations.Core.UploadFile({ file }).then(({ file_url }) => {
        updateSlideImage({ url: file_url, x: 0, y: 0, scale: 1 });
      });
