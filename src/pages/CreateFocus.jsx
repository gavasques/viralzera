...
export default function CreateFocus() {
  ...
  const { data: existingFocus, isLoading: isLoadingData } = useQuery({
    ...
    queryFn: async () => {
      const results = await base44.entities.Focus.filter({ id });
  ...
  const saveMutation = useMutation({
    mutationFn: (data) => {
      if (id) return base44.entities.Focus.update(id, data);
      return base44.entities.Focus.create(data);
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: () => base44.entities.Focus.delete(id),
    onSuccess: async () => {
