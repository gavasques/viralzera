...
export function useChatSession({
  ...
  const { data: config } = useQuery({
    ...
    queryFn: async () => {
      ...
      const configs = await base44.entities[configEntityName].filter({ created_by: user.email });
  ...
  }, [activeSession, config, userConfig, buildSystemPrompt, updateMutation]);
