...
export default function CanvasEditorDialog({ 
  ...
  const { data: history = [], refetch: refetchHistory } = useQuery({
    ...
    queryFn: () => neon.entities.CanvasHistory.filter({ canvas_id: canvas?.id }, '-created_date', 50),
    enabled: !!canvas?.id
  });
  ...
  const handleSaveInternal = async () => {
    if (canvas.content !== editedContent) {
      try {
        await neon.entities.CanvasHistory.create({
          canvas_id: canvas.id,
  ...
  const handleSaveNotes = async (newNotes) => {
    try {
      await neon.entities.Canvas.update(canvas.id, { notes: newNotes });
