...
export default function CanvasNotesPanel({ canvasId }) {
  ...
  const { data: notes = [], isLoading } = useQuery({
    ...
    queryFn: () => neon.entities.CanvasNote.filter({ canvas_id: canvasId }, '-created_date', 100),
    enabled: !!canvasId
  });

  // Create note mutation
  const createMutation = useMutation({
    mutationFn: (content) => neon.entities.CanvasNote.create({
      canvas_id: canvasId,
      content
    }),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => neon.entities.CanvasNote.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['canvasNotes', canvasId] });
