...
export default function PersonaChatSidebar({
  ...
  const SessionItem = ({ session }) => {
    ...
    return (
      ...
      >
        <MessageSquare className={`w-4 h-4 shrink-0 ${isActive ? 'text-indigo-500' : 'text-slate-400'}`} />
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-4 border-b space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="font-semibold text-slate-800">Entrevistas</h2>
          <div className="flex gap-1">
            <Button variant="ghost" size="icon" onClick={onOpenSettings} className="h-8 w-8">
              <Settings className="w-4 h-4" />
            </Button>
            <Button onClick={onNewSession} size="sm" className="bg-indigo-600 hover:bg-indigo-700">
        ...
        <div className="p-2 space-y-4">
          ...
          {filteredSessions.length === 0 && (
            <div className="text-center py-8 text-slate-400 text-sm">
              {searchTerm ? 'Nenhum resultado' : 'Nenhuma entrevista ainda'}
            </div>
          )}
        </div>
      </ScrollArea>
