...
export default function PersonaChatPanel({
  ...
  const { data: configs = [] } = useQuery({
    ...
    queryFn: () => base44.entities.PersonaConfig.list(),
  });
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      const response = await base44.functions.invoke('openrouterChat', {
        ...
        systemPrompt: config?.prompt || "Você é um entrevistador especialista em criar personas. Faça perguntas profundas para entender a pessoa.",
    ...
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <div>
            <h3 className="font-semibold text-slate-800">Gerador de Personas</h3>
            <p className="text-sm text-slate-500 mt-1">
              Selecione ou crie uma entrevista para começar
            </p>
          </div>
          <Button onClick={onNewSession} className="bg-indigo-600 hover:bg-indigo-700">
            <Sparkles className="w-4 h-4 mr-2" /> Nova Entrevista
          </Button>
        </div>
      </div>
    );
