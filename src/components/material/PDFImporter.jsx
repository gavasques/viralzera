...
export default function PDFImporter({ open, onOpenChange, focusId, onSuccess }) {
  ...
  const { data: config } = useQuery({
    ...
    queryFn: async () => {
      const configs = await neon.entities.MaterialBankConfig.list('-created_date', 1);
  ...
  const handleFileSelect = async (e) => {
    ...
    try {
      ...
      const aiResponse = await neon.integrations.Core.InvokeLLM({
        prompt: prompt,
        file_urls: [file_url]
      });
  ...
  const handleSave = async () => {
    ...
    setIsProcessing(true);
    try {
      await neon.entities.Material.create({
        title: result.title,
