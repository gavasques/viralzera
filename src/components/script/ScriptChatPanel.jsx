...
export default function ScriptChatPanel({
  ...
  const { data: scriptConfig } = useQuery({
    ...
    queryFn: async () => {
      const configs = await neon.entities.ScriptConfig.list('-created_date', 1);
      return configs[0];
    },
    staleTime: 60000
  });
  ...
  const handleSend = async (message, files = []) => {
    ...
    const model = session.model || scriptConfig?.config?.model || 'openai/gpt-4o-mini';
    const systemPrompt = scriptConfig?.config?.prompt || '';
    ...
    try {
      ...
      const response = await sendMessage({
        ...
        options: {
          ...
          feature: 'script_chat',
          modelName: session?.model_name || scriptConfig?.config?.model_name || model,
