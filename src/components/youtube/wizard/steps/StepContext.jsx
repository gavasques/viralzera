...
import { WizardCombobox } from "@/components/script/wizard/WizardCombobox";
...
export function StepContext({ focusId, value, onChange }) {
  const { data: personas = [], isLoading: isLoadingPersonas } = useQuery({
    ...
    queryFn: () => neon.entities.Persona.filter({ focus_id: focusId }),
    enabled: !!focusId,
    staleTime: 1000 * 60 * 5,
  ...
  const { data: audienceGroups = [] } = useQuery({
    ...
    queryFn: () => neon.entities.AudienceGroup.filter({ focus_id: focusId }),
    enabled: !!focusId,
    staleTime: 1000 * 60 * 5,
    refetchOnWindowFocus: false
  });
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          <Label className="text-base font-semibold text-slate-900 flex items-center gap-2">
            ...
            Quem vai apresentar? (Persona)
          </Label>
          <p className="text-sm text-slate-500">
            Define a personalidade e tom de voz do apresentador do vídeo.
          </p>
        </div>
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          ...
          <p className="text-sm text-slate-500">
            Foca nas dores e desejos específicos do público do vídeo.
          </p>
        </div>
