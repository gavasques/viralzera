...
export default function AIModelingAnalyzer({ modelingIds, onAnalyze, onCancel }) {
  ...
  const { data: modelings = [], isLoading: loadingModelings } = useQuery({
    ...
    queryFn: async () => {
      ...
      return base44.entities.Modeling.filter({ id: { $in: modelingIds } });
    },
  ...
  const { data: videos = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return base44.entities.ModelingVideo.filter({ modeling_id: { $in: selectedIds } });
    },
  ...
  const { data: texts = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!selectedIds?.length) return [];
      return base44.entities.ModelingText.filter({ modeling_id: { $in: selectedIds } });
    },
    enabled: selectedIds?.length > 0
  });
