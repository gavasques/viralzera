...
export default function ConversationSidebar({ conversations, activeId, onNew, onNewInGroup, onDelete, onSelect, onLoadMore, hasMore }) {
    ...
    const saveGroupMutation = useMutation({
        mutationFn: async ({ id, data }) => {
            if (id) {
                ...
                return base44.entities.TitanosChatGroup.create({ ...data, order: groups.length });
            }
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['titanosChatGroups'] });
    ...
    const deleteGroupMutation = useMutation({
        mutationFn: async (id) => {
            const chatsInGroup = conversations.filter(c => c.group_id === id);
            await Promise.all(chatsInGroup.map(c => 
                base44.entities.TitanosConversation.update(c.id, { group_id: null })
            ));
    ...
    const handleToggleFavorite = (chat) => {
        updateChatMutation.mutate({ 
            chatId: chat.id, 
            data: { is_favorite: !chat.is_favorite } 
        });
        toast.success(chat.is_favorite ? 'Removido dos favoritos' : 'Adicionado aos favoritos!');
    };

    const openGroupSettings = (group = null) => {
            ...
            <Dialog open={isRenameChatModalOpen} onOpenChange={setIsRenameChatModalOpen}>
                <DialogContent className="sm:max-w-[425px]">
                    ...
                    <DialogFooter>
                        ...
                        <Button onClick={() => selectedChat && renameChatMutation.mutate({ id: selectedChat.id, title: inputValue })}>Salvar</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
            ...
            <AlertDialog open={!!deleteGroupTarget} onOpenChange={(open) => !open && setDeleteGroupTarget(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Tem certeza que deseja excluir o grupo "{deleteGroupTarget?.title}"? 
                            As conversas dentro do grupo serão movidas para "Outras Conversas".
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel>Cancelar</AlertDialogCancel>
                        <AlertDialogAction 
                            onClick={() => deleteGroupTarget && deleteGroupMutation.mutate(deleteGroupTarget.id) && setDeleteGroupTarget(null)}
            ...
            <AlertDialog open={!!deleteChatTarget} onOpenChange={(open) => !open && setDeleteChatTarget(null)}>
                <AlertDialogContent>
                    ...
                    <AlertDialogFooter>
                        <AlertDialogCancel>Cancelar</AlertDialogCancel>
                        <AlertDialogAction 
                            onClick={() => deleteChatTarget && onDelete(deleteChatTarget.id) && setDeleteChatTarget(null)}
                            className="bg-red-600 hover:bg-red-700"
                        >
                            Excluir
                        </AlertDialogAction>
