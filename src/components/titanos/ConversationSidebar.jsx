                  ...
                  <Droppable droppableId="favorites">
                    {(provided, snapshot) => (
                      <div
                        ...
                        className={`space-y-0.5 pl-2 ml-1.5 border-l border-slate-100 min-h-[10px] transition-colors ${snapshot.isDraggingOver ? 'bg-amber-50/50 rounded-r-md' : ''
                          }`}
                      >
              ...
              <Droppable droppableId="favorites">
                {(provided, snapshot) => (
                  <div
                    ref={provided.innerRef}
                    {...provided.droppableProps}
                    className={`px-3 py-2 rounded-lg border-2 border-dashed transition-all ${snapshot.isDraggingOver
                  ...
                  <Droppable droppableId={group.id}>
                    {(provided, snapshot) => (
                      <div
                        ref={provided.innerRef}
                        {...provided.droppableProps}
                        className={`space-y-0.5 pl-2 ml-1.5 border-l border-slate-100 min-h-[10px] transition-colors ${snapshot.isDraggingOver ? 'bg-pink-50/50 rounded-r-md' : ''
                          }`}
                      >
              ...
              <Droppable droppableId="ungrouped">
                {(provided, snapshot) => (
                  <div
                    ref={provided.innerRef}
                    {...provided.droppableProps}
                    className={`space-y-1 min-h-[20px] transition-colors ${snapshot.isDraggingOver ? 'bg-slate-100 rounded-lg p-1' : ''
                      }`}
                  >
      ...
      <Dialog open={isRenameChatModalOpen} onOpenChange={setIsRenameChatModalOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription className="sr-only">Altere o título da conversa</DialogDescription>
          </DialogHeader>
          <div className="py-2">
