...
export function useConversationMutations(activeConversationId) {
  ...
  const update = useMutation({
    mutationFn: (data) => {
      ...
      return neon.entities.TitanosConversation.update(activeConversationId, data);
    },
    onSuccess: () => {
  ...
  const remove = useMutation({
    mutationFn: (id) => {
      ...
      return neon.entities.TitanosConversation.delete(id);
    },
  ...
  const create = useMutation({
    mutationFn: async (data) => {
      ...
      const conversation = await neon.entities.TitanosConversation.create(sanitizedData);
...
export function useGroupMutations() {
  ...
  const save = useMutation({
    mutationFn: async ({ id, data }) => {
      ...
      if (id) {
        return neon.entities.TitanosChatGroup.update(id, sanitizedData);
      }
      return neon.entities.TitanosChatGroup.create(sanitizedData);
    },
    onSuccess: (_, { id }) => {
  ...
  const remove = useMutation({
    mutationFn: async (id) => {
      ...
      const conversations = await neon.entities.TitanosConversation.filter({ group_id: id });
      
      if (conversations.length > 0) {
        await Promise.all(
          conversations.map(c => neon.entities.TitanosConversation.update(c.id, { group_id: null }))
        );
      }
      
      return neon.entities.TitanosChatGroup.delete(id);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: TITANOS_QUERY_KEYS.GROUPS });
      queryClient.invalidateQueries({ queryKey: ['titanos-conversations'] });
...
export function useChatMutations() {
  ...
  const updateChat = useMutation({
    mutationFn: ({ chatId, data }) => {
      ...
      return neon.entities.TitanosConversation.update(chatId, data);
    },
    onSuccess: (_, { chatId }) => {
      queryClient.invalidateQueries({ queryKey: ['titanos-conversations'] });
  ...
  const renameChat = useMutation({
    mutationFn: ({ id, title }) => {
      ...
      return neon.entities.TitanosConversation.update(id, { title: sanitizedTitle });
    },
    onSuccess: (_, { id }) => {
      queryClient.invalidateQueries({ queryKey: ['titanos-conversations'] });
...
export function useVoteMutations(conversationId) {
  ...
  const vote = useMutation({
    mutationFn: async ({ modelId, modelAlias, existingVote, allVotes, context }) => {
      ...
      if (existingVote) {
        await neon.entities.ModelVote.delete(existingVote.id);
        return { action: 'removed' };
      }
      
      // Remove votos anteriores (um voto por conversa)
      if (allVotes?.length > 0) {
        await Promise.all(allVotes.map(v => neon.entities.ModelVote.delete(v.id)));
      }
      
      // Cria novo voto
      await neon.entities.ModelVote.create({
